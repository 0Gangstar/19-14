...
# БОЛЬШОЕ ОБНОВЛЕНИЕ!!! (ну вообще нет, но так да)
# Добавлены переходы между локациями ("двери" находятся между двумя деревьями рядом с точкой появления и в доме справа от точки появления
# Перелопачена система отрисовки. Теперь за графическую часть отвчеает не pygame, а OpenGL ( библиотека modenGL)
# Исходя из этого, значительно поднята оптимизация (с 250 фпс в одном месте до +-1700 fps)
# Сильные просадки при столковении с очень сложными хитбоксами
#
#
#
